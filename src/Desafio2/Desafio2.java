			...
			try {
				...
				case 2:
					...
					case 1:
						try {
							...
							if (Integer.valueOf(costmCode) < 1
							...
							System.out.println("Você gostaria de editar/excluir o cliente: "
									+ matrix.get(costmIndex).get(1) + "? \n \n" + "1 - Sim \n" + "2 - Não");
				...
				case 5:
					...
					for (int i = 0; i < quartos.size(); i++) {
						if (quartos.get(i).get(6).equals("f")) {
							System.out.println("Quarto nº:" + quartos.get(i).get(0));
						} else {
							continue;
						}
					}
				...
				case 6:
					...
					if (ans == 1) {
						...
						for (int i = 0; i < quartos.size(); i++) {
							if (quartos.get(i).get(6).equals("t")
									&& quartos.get(i).get(0).equals(Integer.toString(room))) {
						...
						if (quartos.size() == 0) {
							System.out.println("Este quarto não está cadastrado!");
						}
